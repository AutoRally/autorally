...
def get_launch_params(prefix):
    ...
    param_dict["lambda"] = rospy.get_param(prefix + "/lambda")
    param_dict["alpha"] = rospy.get_param(prefix + "/alpha")
...
class Lap:
    ...
    def __init__(self, line, params, prefix):
        ...
        self.params = params
        self.prefix = prefix
        self.pub = rospy.Publisher('lap_stats', pathIntegralStats, queue_size = 1, latch=True)
...
if __name__ == "__main__":
    ...
    while (param_dict is None):
        ...
        if (curr.to_sec() - start.to_sec() > 30.0):
            print("time since started lap stats", curr.to_sec() - start.to_sec())
            break
        try:
