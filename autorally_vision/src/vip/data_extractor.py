...
def interpolate_varying_distances(times, values, target_times):
    result = []
    current_index = 0
    end = target_times[-1]
    for target_time in target_times:
        while current_index < len(times) and times[current_index] < end and times[current_index] < target_time:
            current_index+=1
        if current_index == 0:
            ...
            new_value = interpolate_state(times[current_index-1:current_index+1], values[current_index-1:current_index+1], target_time)
...
def calculate_rate_of_change(times, values, target_times):
    ...
    for target_time in target_times:
        ...
        if current_index == 0:
            ...
            #print "time to interp: ", times[current_index-1:current_index+1], " diff: ", target_time - times[current_index-1], " dt ", times[current_index] - times[current_index-1]
            #print "value to interp: ", values[current_index-1:current_index+1]
            new_value = compute_diff(times[current_index-1:current_index+1], values[current_index-1:current_index+1])
            #print "interped value: ", new_value
            result.append(new_value)
            #print "last time: ", last_time
            #print "times ", times[current_index-1] - last_time, " ", times[current_index] - last_time, " alpha ", alpha
...
def get_all_bag_data(args):
    ...
    if args['dataset'][-3:-1] == "bag":
        ...
        if not os.path.exists(base_path+"dataset_config.yaml"):
            print("cannot find config file ", base_path+"dataset_config.yaml", " creating default")
            config_file = {}
            ...
            yaml.dump(config_file, open(base_path+"dataset_config.yaml", 'w'), default_flow_style=False)
        else:
        ...
        for bag in bags:
            ...
            for key in config_file[bag].keys():
                if key != "dataset":
                    dataset_file_name += "_"+key+"_"+str(config_file[bag][key])
            dataset_file_name += ".pkl"

            print("dataset_file_name", dataset_file_name)

            if os.path.exists(base_path+dataset_file_name) and not args['reload_data']:
                print("loading previsouly extracted dataset file: ", dataset_file_name)
                ...
                result[bag] = get_bag_data(local_params, smooth_data=config_file['smooth'])

                pickle.dump(result[bag], open(base_path+dataset_file_name, 'wb'))
    return result

def interpolate_data(Y, T, spline_pwr = 3, der=0):
    knots = np.linspace(T[0], T[-1], len(T)/3)[1:-1]
    spline_params = interpolate.splrep(np.asarray(T), np.asarray(Y), k = spline_pwr, t=knots)
...
def get_bag_data(args, smooth_data=False):
    ...
    print("search in bag file from ", start_time.to_sec(), " to ", end_time.to_sec())

    #Fill state arrays
    for topic, msg, t in bag_file.read_messages(topics = ['/particle_filter/pose_estimate'], start_time=start_time, end_time=end_time):
        state_ts.append(msg.header.stamp.secs + msg.header.stamp.nsecs/1000000000.0)
    ...
    for topic, msg, t, in bag_file.read_messages(topics = ['/chassisState'], start_time=rospy.Time(start_time.to_sec() - 1), end_time=end_time):
    ...
    new_state_ts = np.linspace(state_ts[0], state_ts[0]+dt*desired_indexes, desired_indexes)
    new_control_ts = np.linspace(state_ts[0], state_ts[0]+dt*desired_indexes, desired_indexes)
    ...
    throttle_cmd = interpolate_varying_distances(control_ts, throttle_cmd, new_control_ts)
    steering_cmd = interpolate_varying_distances(control_ts, steering_cmd, new_control_ts)
    #print "len x: ", len(x)
